...
df <- read_excel("T_vertical_list_updated.xlsx")
...
ggplot() + geom_jitter(data = filter(df, Virus_TF == "T"), 
  ...
  geom_jitter(data = filter(df, Virus_TF == "F"), mapping = aes(x=Sex, y=1e-6),
  ...
  ylim(0,40) +
  ylab("Virus Ct") + xlab("") + 
  ggtitle("Vertical Transmission of Viruses in Aedes Aegypti", subtitle = "Tapachula x New Orleans & Tapachula X Vergel Ae.aegypti Crosses") +
...
theme(strip.text.x = element_text(size = 3.25), 
...
df <- read_excel("T_horizontal_list_updated.xlsx")
...
ggplot() + geom_jitter(data = filter(df, Virus_TF == "T"), 
  ...
  ggtitle("Horizontal Transmission of Viruses in Aedes Aegypti", subtitle = "Tapachula x New Orleans & Tapachula X Vergel Ae.aegypti Crosses") +
  scale_color_manual(values=c("#66CC99", "#56B4E9")) +
  scale_fill_manual(values=c("#66CC99", "#56B4E9")) +
  theme(legend.position = "none") + theme_grey() + theme(legend.position = "none") +
  theme(strip.text.x = element_text(size = 3.25), 
